-- =====================================================
-- 1. Удаление схемы northwind, если она существует
-- =====================================================
...
-- =====================================================
-- 3. Установка схемы по умолчанию
-- =====================================================
...
-- Отключает ограничение времени выполнения SQL-запросов (запросы могут выполняться неограниченно долго)
SET statement_timeout = 0;

-- Отключает таймаут ожидания блокировок (ожидание блокировки — бесконечное)
SET lock_timeout = 0;

-- Устанавливает кодировку клиентского соединения в UTF-8 (корректная работа с Unicode и кириллицей)
SET client_encoding = 'UTF8';

-- Включает стандартную обработку строк SQL:
-- символ '\' не является escape-символом, для экранирования используется E'...'
SET standard_conforming_strings = on;

-- Отключает проверку тела функций при создании
-- (позволяет создавать функции, ссылающиеся на ещё не существующие объекты)
SET check_function_bodies = false;

-- Отображает клиенту только предупреждения и ошибки,
-- скрывая информационные и отладочные сообщения
SET client_min_messages = warning;


-- Использует tablespace базы данных по умолчанию
-- (не привязывает создаваемые объекты к пользовательским tablespace)
SET default_tablespace = '';

-- Отключает автоматическое добавление системного столбца OID в таблицы
-- (OID устарели и не рекомендуются к использованию)
